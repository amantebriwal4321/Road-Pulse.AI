...
const MunicipalityMap = () => {
  ...
  const stats = useMemo(() => {
    ...
    const avgSev =
      total > 0 ? potholes.reduce((s, p) => s + p.severity, 0) / total : 0;
    const coverage = Math.min(100, Math.round((total / 300) * 100));
    return [
      {
        label: "ACTIVE POTHOLES",
        value: total.toLocaleString(),
        trend: `+${Math.floor(total * 0.05)}`,
        up: true,
      },
      {
        label: "CRITICAL ZONES",
        value: String(critical),
        trend: `+${Math.max(1, Math.floor(critical * 0.1))}`,
        up: true,
      },
      {
        label: "HIGH SEVERITY",
        value: String(high),
        trend: `+${Math.max(1, Math.floor(high * 0.08))}`,
        up: true,
      },
      {
        label: "AVG SEVERITY",
        value: avgSev.toFixed(1),
        trend: avgSev > 6 ? "+0.3" : "-0.2",
        up: avgSev > 6,
      },
      {
        label: "DATA POINTS (24H)",
        value: totalReports.toLocaleString(),
        trend: "+" + Math.floor(totalReports * 0.02).toLocaleString(),
        up: true,
      },
      {
        label: "TWIN COVERAGE",
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const template =
        tickerTemplates[Math.floor(Math.random() * tickerTemplates.length)];
    ...
    <div className="min-h-screen bg-metaverse-grid relative pb-24">
      ...
      <div className="relative z-10">
        ...
        <div className="glass-card m-0 rounded-none px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <HUDLabel className="border-electric-blue/30 text-electric-blue hidden md:inline-block">
              WARD OFFICER — WARD 52
            </HUDLabel>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-80 p-4 space-y-4 overflow-y-auto lg:h-[calc(100vh-60px)]">
            {/* Stats — LIVE from API */}
            <div>
              <HUDLabel className="border-electric-blue/30 text-electric-blue mb-3">
                ACTIVE METRICS · LIVE
              </HUDLabel>
                ...
                {stats.map((m) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-sm font-medium text-cyan font-bold">
                        {m.value}
                      </span>
                      <span
                        className={`text-[10px] font-mono flex items-center ${m.up ? "text-alert" : "text-green"}`}
                      >
                        {m.up ? (
            ...
            <div>
              <HUDLabel className="border-alert/30 text-alert mb-3">
                EMERGENCY QUEUE · LIVE
              </HUDLabel>
              <div className="space-y-2 mt-3">
                  ...
                  <GlassCard key={e.id} className="p-3" nohover>
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-foreground text-sm font-medium font-display truncate">
                          {e.roadName}
                        </p>
                        <p className="text-text-secondary text-[10px] font-mono">
                          {e.ward} · {e.severityScore.toFixed(1)} ·{" "}
                          {e.reportCount} reports
                        </p>
                      </div>
                    </div>
                    {!dispatchedIds.has(e.id) ? (
                      <NeonButton
                        size="sm"
                        variant="primary"
                        className="w-full mt-2 bg-alert hover:shadow-[0_0_16px_hsl(var(--alert)/0.4)] text-xs font-medium"
                        onClick={() =>
                          setDispatchedIds((prev) => new Set(prev).add(e.id))
                        }
                      >
                        DISPATCH
                      </NeonButton>
                    ) : (
                      <HUDLabel className="mt-2 border-amber/30 text-amber">
                        CREW DISPATCHED
                      </HUDLabel>
                    )}
                  </GlassCard>
              ...
              <div className="space-y-2 mt-3">
                <AnimatePresence>
                  {tickerItems.map((t, i) => (
                    ...
                    >
                      {t}
                      <p className="text-text-secondary text-[9px] font-mono mt-1">
                        {i === 0 ? "Just now" : `${i * 5}m ago`}
                      </p>
                    </motion.div>
                  ))}
                </AnimatePresence>
