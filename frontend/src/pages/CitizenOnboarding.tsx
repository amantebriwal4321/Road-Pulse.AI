    ...
    return (
      <div className="min-h-screen bg-metaverse-grid flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="w-full h-1 bg-gradient-to-r from-transparent via-cyan to-transparent animate-scanning" />
        </div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center z-10">
          <motion.p initial={{ opacity: 1 }} animate={{ opacity: 0 }} transition={{ delay: 1.5, duration: 0.5 }}
            className="font-mono text-cyan text-xl tracking-widest">TWIN INITIALISING...</motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2, duration: 0.5 }}
            className="font-mono text-cyan text-3xl tracking-widest font-bold">TWIN ACTIVE</motion.p>
        </motion.div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-mono border ${s <= step ? 'bg-cyan/20 border-cyan text-cyan' : 'border-border-glow text-text-secondary'}`}>
                {s < step ? <Check className="w-4 h-4" /> : s}
              </div>
              {s < 4 && <div className={`w-8 h-px ${s < step ? 'bg-cyan' : 'bg-border-glow'}`} />}
            </div>
          ))}
        </div>
          ...
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="space-y-3">
                <input placeholder="Full name" className="w-full px-4 py-3 bg-surface border border-border-glow rounded-lg text-foreground placeholder:text-text-secondary text-sm focus:outline-none focus:border-cyan/50" />
                <input placeholder="Email" type="email" className="w-full px-4 py-3 bg-surface border border-border-glow rounded-lg text-foreground placeholder:text-text-secondary text-sm focus:outline-none focus:border-cyan/50" />
                <input placeholder="Phone number" className="w-full px-4 py-3 bg-surface border border-border-glow rounded-lg text-foreground placeholder:text-text-secondary text-sm focus:outline-none focus:border-cyan/50" />
                <select className="w-full px-4 py-3 bg-surface border border-border-glow rounded-lg text-foreground text-sm focus:outline-none focus:border-cyan/50">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {vehicleTypes.map((v) => (
                  <button key={v.id} onClick={() => setVehicleType(v.id)}
                    className={`p-4 rounded-lg border flex flex-col items-center gap-2 transition-all cursor-pointer ${vehicleType === v.id ? 'border-cyan bg-cyan/10 text-cyan' : 'border-border-glow text-text-secondary hover:border-cyan/30'}`}>
              ...
              <div className="flex gap-2 mb-4">
                {['Budget', 'Mid-range', 'Flagship'].map((tier) => (
                  <button key={tier} onClick={() => setPhoneTier(tier.toLowerCase())}
                    className={`flex-1 py-3 rounded-lg border text-sm transition-all cursor-pointer ${phoneTier === tier.toLowerCase() ? 'border-cyan bg-cyan/10 text-cyan' : 'border-border-glow text-text-secondary'}`}>
                    {tier}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-2 mb-6 mt-4">
                {['Pocket', 'Cup Holder', 'Dashboard', 'Handlebar'].map((pos) => (
                  <button key={pos} onClick={() => setMountPos(pos.toLowerCase())}
                    className={`flex-1 py-2 rounded-lg border text-xs transition-all cursor-pointer ${mountPos === pos.toLowerCase() ? 'border-cyan bg-cyan/10 text-cyan' : 'border-border-glow text-text-secondary'}`}>
                    {pos}
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-3 mb-6">
                {['Danger ahead alerts', 'Route intelligence', 'Weekly twin report'].map((label) => (
                  <label key={label} className="flex items-center justify-between">
                    ...
                    <div className="w-10 h-5 bg-cyan/20 rounded-full relative cursor-pointer">
                      <div className="w-4 h-4 bg-cyan rounded-full absolute top-0.5 right-0.5" />
                    </div>
                  </label>
                ))}
              </div>
