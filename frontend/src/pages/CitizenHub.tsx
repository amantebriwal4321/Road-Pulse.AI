...
function BottomNav({ active, dark = false }: { active: string; dark?: boolean }) {
        ...
        {items.map((item) => (
          <button
            ...
            style={{
              ...
              gap: 3, cursor: 'pointer', border: 'none', background: 'transparent',
...
function StatCard({
  ...
  icon: React.ElementType; label: string; value: string | number; color: string; dark: boolean;
}) {
  return (
    <div style={{
      ...
      <div>
        <p style={{
          margin: 0, fontSize: 16, fontWeight: 700, lineHeight: 1.1,
        ...
        <p style={{
          margin: 0, fontSize: 8, fontFamily: 'monospace',
...
const CitizenHub = () => {
  ...
  const sevStats = useMemo(() => {
    ...
    const med = potholes.filter((p) => p.severity >= 4 && p.severity < 7).length;
  ...
  const checkProximity = useCallback(() => {
    ...
    const label = closest.severity >= 7 ? 'HIGH' : closest.severity >= 4 ? 'MEDIUM' : 'LOW';
  ...
  /* ═══════════════════════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════════════════════ */
  return (
    <div style={{ position: 'fixed', inset: 0, background: d ? '#0f172a' : '#f8fafc' }}>
      ...
      <AnimatePresence>
        {simActive && (
          <SimHUD simState={simState} onStop={handleStopSim} />
        )}
      </AnimatePresence>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{
              ...
              fontWeight: 700, color: textSecondary, margin: 0,
            }}>
              ROADPULSE TWIN HUB
            </p>
            <p style={{
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            ...
            }}>{sevStats.high} HIGH</span>
            <span style={{
              padding: '3px 8px', borderRadius: 6, fontSize: 9,
              fontFamily: 'monospace', fontWeight: 700,
              background: 'rgba(245,158,11,0.12)', color: '#f59e0b',
            }}>{sevStats.med} MED</span>
            <span style={{
              padding: '3px 8px', borderRadius: 6, fontSize: 9,
              fontFamily: 'monospace', fontWeight: 700,
              background: 'rgba(34,197,94,0.12)', color: '#22c55e',
            }}>{sevStats.low} LOW</span>
          </div>
          ...
          >
            {d ? <Sun size={14} color="#fbbf24" /> : <Moon size={14} color="#6b7280" />}
          </button>
          ...
          <button
            ...
            style={{
              ...
              background: autoDetect
                ? 'rgba(37,99,235,0.12)'
                : d ? 'rgba(255,255,255,0.06)' : 'rgba(156,163,175,0.1)',
      ...
      {!simActive && (geoError || usingDemoLocation) && (
        <div style={{
          position: 'absolute', top: 52, left: 12, right: 12, zIndex: 1100,
      ...
      <AnimatePresence>
        {proximityAlert && (
          <motion.div
            ...
            exit={{ y: -60, opacity: 0 }}
            style={{
              position: 'absolute', top: 52, left: 12, right: 12, zIndex: 2000,
            }}
          >
            <ProximityAlert
      ...
      {/* ── Right FABs ──────────────────────────────────────────── */}
      <div style={{
        position: 'absolute', right: 12, bottom: 100, zIndex: 1100,
        ...
        >
          <Building2 size={16} color={d ? 'rgba(255,255,255,0.5)' : '#6b7280'} />
        </button>
      </div>

      {/* ── Left panels (stacked, no overlap) ───────────────────── */}
      <div style={{
        position: 'absolute', left: 12, bottom: 88, zIndex: 1100,
        ...
        {!simActive && (
          ...
          >
            ...
            <div style={{ textAlign: 'left' }}>
              <p style={{ margin: 0, fontSize: 10, fontWeight: 700, letterSpacing: '0.05em' }}>
                DEMO SIMULATION
              </p>
      ...
      {/* ── Advisory pill ───────────────────────────────────────── */}
      <div style={{
        position: 'absolute', bottom: 62, left: 12, right: 12, zIndex: 1100,
        background: glassBg, backdropFilter: 'blur(14px)',
        border: `1px solid ${glassBorder}`, borderRadius: 10,
        padding: '7px 12px',
        display: 'flex', alignItems: 'center', gap: 6,
      }}>
        <div style={{
          width: 6, height: 6, borderRadius: '50%', flexShrink: 0,
          background: potholes.some((p) => p.severity >= 7) ? '#ef4444' : '#2563eb',
        ...
        <span style={{
          fontSize: 10, color: textPrimary, flex: 1, textAlign: 'right',
          fontFamily: 'monospace', fontWeight: 600,
        }}>
          {advisoryText}
        </span>
